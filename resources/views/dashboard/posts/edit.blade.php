<x-app-layout>
            ...
            <div class="col-md-9 col-lg-10">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        ...
                        <p class="mb-0 text-muted">Update your blog post with drag and drop elements</p>
                    </div>
                        ...
                        <form id="blogPostForm">
                                ...
                                <div class="col-md-8">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="title" name="title" required
                                                placeholder="Enter your blog post title" value="{{ old('title', $blogPost->title) }}">
                                    </div>
                                </div>
                                <div class="col-md-4">
                                    <div class="mb-3">
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="is_featured" name="is_featured">
                                            <option value="0" {{ old('is_featured', $blogPost->is_featured) == '0' ? 'selected' : '' }}>No</option>
                                            <option value="1" {{ old('is_featured', $blogPost->is_featured) == '1' ? 'selected' : '' }}>Yes</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="excerpt" name="excerpt" rows="2" 
                                            placeholder="Brief description of your blog post">{{ old('excerpt', $blogPost->excerpt) }}</textarea>
                            </div>

                            <div class="mb-3">
                                <label for="tags" class="form-label">Tags</label>
                                <div class="tags-container" id="tagsContainer">
                                    @if($blogPost->tags && is_array($blogPost->tags))
                                ...
                                <input type="hidden" id="tags" name="tags" value="{{ $blogPost->tags ? implode(',', $blogPost->tags) : '' }}">
                            </div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="meta_title" name="meta_title" 
                                                placeholder="Meta title for SEO" value="{{ old('meta_title', $blogPost->meta_title) }}">
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <div class="mb-3">
                            ...
                            <div class="mb-3">
                                ...
                                <textarea class="form-control" id="meta_description" name="meta_description" rows="2" 
                                            placeholder="Meta description for SEO">{{ old('meta_description', $blogPost->meta_description) }}</textarea>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <div class="canvas-area" id="canvas">
                            @if(!$blogPost->structure || !isset($blogPost->structure['elements']) || count($blogPost->structure['elements']) === 0)
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <div class="d-flex justify-content-between align-items-center">
                            <div>
                                <h6 class="mb-1">Ready to update your blog post?</h6>
    ...
    @push('vendor-js')
    <!-- Scripts (bootstrap bundle already loaded by the dashboard layout) -->
    ...
    <script>
        ...
        class TagsManager {
            ...
            init() {
                ...
                if (existingTags) {
                    this.tags = existingTags.split(',').filter(tag => tag.trim() !== '');
                    this.renderTags();
                }
        ...
        class BlogPostBuilder {
            ...
            loadExistingElements() {
                @if($blogPost->structure && isset($blogPost->structure['elements']))
                    const existingElements = @json($blogPost->structure['elements']);
                    ...
                    // Update nextId based on existing elements
                    if (this.elements.length > 0) {
                        const lastId = this.elements[this.elements.length - 1].id;
                ...
                const templates = {
                    ...
                    image: (el) => `
                        <div class="canvas-element" data-id="${el.id}">
                            ...
                            ${el.content.src ? `<img src="${this.getImageUrl(el.content.src)}" class="img-fluid mt-2" style="max-height: 200px;">` : ''}
                        </div>
                    `,
            ...
            getImageUrl(path) {
                ...
                if (path.startsWith('blog/') || path.startsWith('blog-images/')) {
                    return '/website/' + path;
                }
                return path;
            }
            ...
            renderPreviewElement(element) {
                const templates = {
                    ...
                    image: (el) => `
                        <div class="mb-4">
                            ${el.content.src ? `<img src="${this.getImageUrl(el.content.src)}" class="img-fluid rounded mb-2" style="max-height: 300px;">` : '<div class="bg-light text-center py-5 rounded text-muted">No image</div>'}
                    ...
                    banner: (el) => `
                        <div class="bg-light p-5 mb-4 text-center rounded">
                            ${el.content.src ? `<img src="${this.getImageUrl(el.content.src)}" class="img-fluid mb-3" style="max-height: 200px;">` : ''}
            ...
            async saveBlogPost() {
                ...
                try {
                    const response = await fetch('{{ route("admin.blog-posts.update", $blogPost) }}', {
                        method: 'POST',
                        body: formData,
                        headers: {
