...
class OrderController extends Controller
{
    public function index(Request $request)
    {
        ...
        $query = Order::with(['user', 'shop', 'coupon'])
        ...
        if (auth()->user()->hasRole('shop-owner')) {
            ...
            if (! $shop) {
                $orders = Order::query()->whereRaw('0 = 1')->paginate(10)->withQueryString();
        ...
        if ($request->filled('payment_status')) {
            ...
            if (in_array($paymentStatus, ['pending', 'paid', 'failed', 'refunded', 'partially_refunded'], true)) {
                $query->where('payment_status', $paymentStatus);
            }
        }

        if ($request->filled('shop_id') && auth()->user()->hasRole('super-admin')) {
        ...
        if ($search !== '') {
            $searchableColumns = [
                'order_number', 'shipping_email', 'shipping_phone', 'uuid', 'transaction_id',
        ...
        $allowedSort = [
            'id', 'order_number', 'total_amount', 'status', 'payment_status', 'payment_method',
            'created_at', 'shop', 'shipping_last_name',
        ];
